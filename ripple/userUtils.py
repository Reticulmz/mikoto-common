...
def submitBeatmapRequest(username, beatmapID, mapType):
	...
	embed.set_author(name=username, icon='http://a.revipsu.cf/{}'.format(userID), url="http://revipsu.cf/u/{}".format(userID))
	embed.set_image('https://assets.ppy.sh/beatmaps/{}/covers/cover.jpg?1522396856'.format(beatmap['beatmapset_id']))
	embed.set_title(title="{}".format(beatmap['song_name']), url="http://revipsu.cf/b/{}".format(beatmapID))
	embed.set_footer(text='beatmap nomination system ', icon='https://i.namir.in//sYi.png', ts=True)
	embed.add_field(name='This **{}** has been nominated by {}.'.format(mapType, username), value='** **')
	embed.post()
